...
describe("ConfigEditor", () => {
  ...
  it("tracks canned message drafts separately from module config", () => {
    const { transport } = createFakeTransport();
    const client = new MeshClient({ transport });
    const editor = client.config.editor;

    client.events.onCannedMessageModulePacket.dispatch({
      id: 1,
      rxTime: new Date(),
      from: 0,
      to: 0,
    ...
    expect(editor.cannedMessageModuleMessages.value).toBe("Hello|SOS");
    expect(editor.isDirty.value).toBe(false);

    editor.setCannedMessageModuleMessages("Hello|SOS|Help");

    expect(editor.cannedMessageModuleMessages.value).toBe("Hello|SOS|Help");
    expect(editor.isDirty.value).toBe(true);
  ...
  it("tracks ringtone drafts separately from module config", () => {
    const { transport } = createFakeTransport();
    const client = new MeshClient({ transport });
    const editor = client.config.editor;

    client.events.onRingtonePacket.dispatch({
      id: 2,
      rxTime: new Date(),
      from: 0,
      to: 0,
      type: "direct",
      channel: 0,
      data: "Bubble:d=16,o=5,b=140:a6,b6,d7",
    });

    expect(editor.ringtoneMessage.value).toBe(
      "Bubble:d=16,o=5,b=140:a6,b6,d7",
    );
    expect(editor.isDirty.value).toBe(false);

    editor.setRingtoneMessage("Mario:d=4,o=5,b=100:c,e,g");

    expect(editor.ringtoneMessage.value).toBe("Mario:d=4,o=5,b=100:c,e,g");
    expect(editor.isDirty.value).toBe(true);
