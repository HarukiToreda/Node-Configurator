...
export class ConfigEditor {
  ...
  private readonly baselineCannedMessageModuleMessages = signal<string>("");
  private readonly workingCannedMessageModuleMessages = signal<string>("");
  private readonly baselineRingtoneMessage = signal<string>("");
  private readonly workingRingtoneMessage = signal<string>("");
  ...
  public readonly cannedMessageModuleMessages: ReadonlySignal<string> =
    toReadonly(this.workingCannedMessageModuleMessages);
  public readonly ringtoneMessage: ReadonlySignal<string> = toReadonly(
    this.workingRingtoneMessage,
  );
  ...
  constructor(client: MeshClient) {
    ...
    client.events.onCannedMessageModulePacket.subscribe((packet) => {
      this.baselineCannedMessageModuleMessages.value = packet.data;
      if (!this.hasQueuedAdminMessageCase("setCannedMessageModuleMessages")) {
        this.workingCannedMessageModuleMessages.value = packet.data;
      }
      this.recomputeDirty();
    });

    client.events.onRingtonePacket.subscribe((packet) => {
      this.baselineRingtoneMessage.value = packet.data;
      if (!this.hasQueuedAdminMessageCase("setRingtoneMessage")) {
        this.workingRingtoneMessage.value = packet.data;
      }
      this.recomputeDirty();
    });
    ...
    client.events.onDeviceStatus.subscribe((status) => {
      if (status === DeviceStatusEnum.DeviceDisconnected) {
        ...
        this.baselineCannedMessageModuleMessages.value = "";
        this.baselineRingtoneMessage.value = "";
        this.workingRadio.value = {};
        this.workingModules.value = {};
        this.workingChannels.value = new Map();
        this.workingOwner.value = undefined;
        this.workingCannedMessageModuleMessages.value = "";
        this.workingRingtoneMessage.value = "";
  ...
  public setCannedMessageModuleMessages(messages: string): void {
    this.workingCannedMessageModuleMessages.value = messages;
    this.syncCannedMessageModuleMessagesQueue(messages);
    this.recomputeDirty();
  }

  public setRingtoneMessage(message: string): void {
    this.workingRingtoneMessage.value = message;
    this.syncRingtoneMessageQueue(message);
    this.recomputeDirty();
  }
  ...
  public replaceQueuedAdminMessages(
    cases: readonly Protobuf.Admin.AdminMessage["payloadVariant"]["case"][],
    messages: readonly Protobuf.Admin.AdminMessage[],
  ): void {
    const removeCases = new Set(cases);
    this.queuedAdminMessages.value = [
      ...this.queuedAdminMessages
        .peek()
        .filter(
          (message) =>
            !message.payloadVariant.case ||
            !removeCases.has(message.payloadVariant.case),
        ),
      ...messages,
    ];
    if (removeCases.has("setCannedMessageModuleMessages")) {
      const queuedCannedMessage = messages.find(
        (message) =>
          message.payloadVariant.case === "setCannedMessageModuleMessages",
      );
      this.workingCannedMessageModuleMessages.value =
        queuedCannedMessage?.payloadVariant.case ===
        "setCannedMessageModuleMessages"
          ? queuedCannedMessage.payloadVariant.value
          : this.baselineCannedMessageModuleMessages.peek();
    }
    if (removeCases.has("setRingtoneMessage")) {
      const queuedRingtoneMessage = messages.find(
        (message) => message.payloadVariant.case === "setRingtoneMessage",
      );
      this.workingRingtoneMessage.value =
        queuedRingtoneMessage?.payloadVariant.case === "setRingtoneMessage"
          ? queuedRingtoneMessage.payloadVariant.value
          : this.baselineRingtoneMessage.peek();
    }
    this.recomputeDirty();
  }
  ...
  public reset(): void {
    ...
    this.workingCannedMessageModuleMessages.value =
      this.baselineCannedMessageModuleMessages.peek();
    this.workingRingtoneMessage.value = this.baselineRingtoneMessage.peek();
    this.queuedAdminMessages.value = [];
    this.recomputeDirty();
  }
  ...
  public async commit(): Promise<ResultType<void, Error>> {
    ...
    this.baselineCannedMessageModuleMessages.value =
      this.workingCannedMessageModuleMessages.peek();
    this.baselineRingtoneMessage.value = this.workingRingtoneMessage.peek();
    this.queuedAdminMessages.value = [];
    this._dirtyRadioSections.value = [];
    this._dirtyModuleSections.value = [];
    this._dirtyChannels.value = [];
  ...
  private recomputeDirty(): void {
    ...
    const cannedMessagesDirty =
      this.baselineCannedMessageModuleMessages.peek() !==
      this.workingCannedMessageModuleMessages.peek();
    const ringtoneDirty =
      this.baselineRingtoneMessage.peek() !== this.workingRingtoneMessage.peek();
  ...
  private hasQueuedAdminMessageCase(
    targetCase: Protobuf.Admin.AdminMessage["payloadVariant"]["case"],
  ): boolean {
    return this.queuedAdminMessages
      .peek()
      .some((message) => message.payloadVariant.case === targetCase);
  }

  private syncCannedMessageModuleMessagesQueue(messages: string): void {
    const nextMessages = this.queuedAdminMessages
      .peek()
      .filter(
        (message) =>
          message.payloadVariant.case !== "setCannedMessageModuleMessages",
    ...
    if (messages !== this.baselineCannedMessageModuleMessages.peek()) {
      nextMessages.push(
    ...
    this.queuedAdminMessages.value = nextMessages;
  }

  private syncRingtoneMessageQueue(message: string): void {
    const nextMessages = this.queuedAdminMessages
      .peek()
      .filter((queuedMessage) => queuedMessage.payloadVariant.case !== "setRingtoneMessage");

    if (message !== this.baselineRingtoneMessage.peek()) {
      nextMessages.push(
        create(Protobuf.Admin.AdminMessageSchema, {
          payloadVariant: {
