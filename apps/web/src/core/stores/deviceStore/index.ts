...
const SERIAL_LOG_RETENTION_NUM = 2000;
...
export interface Device extends DeviceData {
  ...
  addSerialLogEntry: (entry: SerialLogEntry) => void;
    ...
    addSerialLogEntry: (entry) => {
      set(
        produce<PrivateDeviceState>((draft) => {
          ...
          device.serialLogs.push(entry);
          evictOldestEntries(device.serialLogs, SERIAL_LOG_RETENTION_NUM);
        }),
      );
    },
    clearSerialLogs: () => {
